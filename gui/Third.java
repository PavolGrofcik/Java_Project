...
public class Third extends Stage  {
	...
	//pomocnÈ premennÈ sl˙ûiace len na overenie zlavovÈho kupÛnu
	private int pom =0;
	private int num =0;
	...
	public Third(Objedn·vka parameter){																//parametrom je arralist s objektami rÙznych kontajnerov, v tejto Ëasti sa dokonËÌ objedn·vka: t.j zistÌ celkovÈ ceny, Ëas...
		...
		cena.setOnAction(e->{
			...
			num++;
				...
				vypis.clear();
				vypis.appendText("**********\nMesto: " + objedn·vka.getMesto() + "\n" + "Vzdialenosù: " + Integer.toString(objedn·vka.getVzdialenost()) + " km" +
						"\n" + "Cena: " + Integer.toString(objedn·vka.getTotalCena()) + " Ä" + "\n" + "PoËet dnÌ: " + Integer.toString(objedn·vka.getPocetDni()) + 
		...
		odosli.setOnAction(e->{
			...
			//Vypne program po 5 sekund·ch odkliknutÌm tlaËidla odosli
		...
		zmenCenu.setOnAction(e->{
			 ...
			 if (pom == 0 && objedn·vka.zlavovyKupon(kupon.getText())!=0) {
				vypis.clear();
				vypis.appendText("Nov· cena: " + Integer.toString(objedn·vka.zlavovyKupon(kupon.getText())) + "Ä");
				kupon.clear();
				pom++;
			}
			else{
